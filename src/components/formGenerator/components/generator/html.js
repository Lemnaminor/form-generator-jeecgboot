...
const tags = {
  'j-select-user': el => {
    ...
    const multi = el.multi ? `:multi="${!!el.multi}"` : ''

    return `<${tag} ${vModel} ${multi}></${tag}>`
  },
  'j-select-dept': el => {
    const {
      tag, vModel
  ...
  'j-text': el => {
    ...
    const style = `width:100%;color:${el.color};text-align:${el.align};font-size:${el.fontSize}px;font-weight:${el.fontWeight ? 'bold' : ''};
    font-style:${el.fontStyle ? 'italic' : ''};line-height:${el.lineHeight};text-decoration:${el.fontDecoration ? 'underline' : ''}`

    // const valueText = el['value-text'] ? `value-text="${el['value-text']}"` : ''
    const color = el.color ? `color="${el.color}"` : ''
    const align = el.align ? `text-align="${el.align}"` : ''
    const fontSize = el['font-size'] ? `font-size="${el['font-size']}"` : ''
    const lineHeight = el['line-height'] ? `line-height="${el['line-height']}"` : ''
    const fontWeight = el['font-weight'] ? `:font-weight="${!!el['font-weight']}"` : ''
    const fontStyle = el['font-style'] ? `:font-style="${!!el['font-style']}"` : ''
    const fontDecoration = el['font-decoration'] ? `:font-decoration="${!!el['font-decoration']}"` : ''

    return `<${tag} ${vModel} ${color} ${align} ${fontSize} ${lineHeight} ${fontWeight} ${fontStyle} ${fontDecoration}></${tag}>`
  },
  'el-divider': el => {
    const {
      tag
    ...
    const contentPosition = el['content-position'] ? `contentPosition="${el['content-position']}"` : ''
    let child = buildElDividerChild(el)

    if (child) child = `\n${child}\n` // 换行
    return `<${tag} ${direction} ${contentPosition}>${child}</${tag}>`
  },
  'el-button': el => {
    const {
      tag, disabled
...
// el-divider 子级
function buildElDividerChild(scheme) {
  const children = []
